...
public class TimeDAO {
    ...
    public void create(Time time) throws SQLException {
        String sql = "INSERT INTO times (nome, cidade, estado, ano_fundacao, tecnico, vitorias) VALUES (?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, time.getNome());
            stmt.setString(2, time.getCidade());
            stmt.setString(3, time.getEstado());
            stmt.setInt(4, time.getAnoFundacao());
            stmt.setString(5, time.getTecnico());
            stmt.setInt(6,time.getVitorias());
            stmt.executeUpdate();
        }
    }
    ...
    public void update(Time time) throws SQLException {
        String sql = "UPDATE times SET nome = ?, cidade = ?, estado = ?, ano_fundacao = ?, tecnico = ? WHERE id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, time.getNome());
            stmt.setString(2, time.getCidade());
            stmt.setString(3, time.getEstado());
